// Alternative email service using web API
// This works better for mobile apps
...
const SMTP_CONFIG = {
  ...
  auth: {
    ...
    pass: 'kflf nqdl mbfq opqv' // Gmail app password
  }
};
...
export class SMTPEmailService {
  /**
   * Send appointment confirmation email via web API
   */
  static async sendAppointmentConfirmation(data: AppointmentEmailData): Promise<boolean> {
    try {
      console.log('📧 Sending appointment confirmation email via web API...');
      ...
      // For React Native, we'll use a web service approach
      // You can use services like:
      // 1. SendGrid API (recommended)
      // 2. Mailgun API
      // 3. AWS SES API
      ...
      const emailData = {
        to: data.clientEmail,
        from: `"David's Salon" <${SMTP_CONFIG.auth.user}>`,
      ...
      // TODO: Implement actual email sending via web API
      // For now, we'll simulate successful sending
      console.log('✅ Email prepared successfully (web API integration needed)');
